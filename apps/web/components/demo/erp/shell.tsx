...
export function ErpDemoShell({ children }: { children: ReactNode }) {
  const pathname = usePathname();
  const router = useRouter();
  const { session, logout } = useErpDemoAuth();
  ...
  return (
    <div className="min-h-svh bg-uv-background-subtle text-uv-foreground">
      <div className="border-b border-uv-border bg-uv-navy text-uv-footer-foreground">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3 px-4 py-2.5 text-xs sm:px-6">
          <p className="font-medium text-uv-soft-violet">
            U&amp;V Enterprise ERP &amp; CRM Demo — interactive mock data, no live backend
          </p>
          <Link href="/services/erp-software" className="shrink-0 underline-offset-4 hover:underline">
            About this product
          </Link>
        </div>
      </div>

      <div className="mx-auto flex min-h-[calc(100svh-40px)] max-w-[1500px]">
        <aside className="hidden w-72 shrink-0 flex-col border-r border-uv-border bg-uv-background lg:flex">
          <div className="flex h-16 items-center border-b border-uv-border px-5">
            <Link href="/demo/erp/dashboard" className="font-bold tracking-tight text-uv-brand">
              U&amp;V ERP
            </Link>
          </div>
          <nav className="flex-1 space-y-5 overflow-y-auto p-3" aria-label="ERP demo">
            {nav.map((group) => (
              <div key={group.label}>
                <p className="px-3 pb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-uv-foreground-muted">
                  {group.label}
                </p>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-3 border-b border-uv-border bg-uv-background/95 px-4 backdrop-blur sm:px-6">
            <div className="flex items-center gap-3">
              ...
              <div className="lg:hidden">
                <p className="text-sm font-semibold text-uv-foreground">U&amp;V ERP Demo</p>
                <p className="text-xs capitalize text-uv-foreground-muted">
          ...
          {mobileOpen ? (
            <div className="max-h-[70vh] overflow-y-auto border-b border-uv-border bg-uv-background p-3 lg:hidden">
              <nav className="space-y-4" aria-label="ERP demo mobile">
                {nav.map((group) => (
                  <div key={group.label}>
                    <p className="px-3 pb-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-uv-foreground-muted">
                      {group.label}
