  ...
  const tone =
    ...
    status === 'free'
      ...
      : status === 'placed' ||
        ...
        : status === 'cancelled' || status === 'refunded' || status === 'off'
    ...
    <div className="flex h-56 items-end gap-3">
      {data.map((row) => (
        <div key={row.label} className="flex flex-1 flex-col items-center gap-2">
          <p className="text-xs font-medium text-uv-foreground-muted">{row.value}k</p>
...
export function DietPill({ diet }: { diet: string }) {
  return (
    <span
      className={cn(
        'inline-flex h-4 w-4 items-center justify-center rounded-sm border text-[8px] font-bold',
        diet === 'veg'
