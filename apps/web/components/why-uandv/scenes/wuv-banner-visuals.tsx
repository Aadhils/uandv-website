...
function BannerShell({
  ...
  return (
    <WuvMicroSceneShell
      ...
      activeClassName={reduced ? '' : 'is-banner-active'}
      threshold={0.15}
    >
...
/** Origin — vendor vs partner before/after micro-scene */
  ...
  return (
    <BannerShell label="Vendor problems versus U&V partnership outcomes" className={className}>
      <svg viewBox="0 0 560 300" className="h-full w-full" preserveAspectRatio="xMidYMid slice" aria-hidden>
        {/* Before — fragmented vendor experience */}
        <rect x="16" y="16" width="256" height="268" rx="12" fill="#FEF2F2" fillOpacity="0.45" stroke="#FECACA" strokeWidth="1.2" />
        <WuvMicroFloatG delay={0} duration="6s">
          <g>
            ...
            <rect x="140" y="120" width="72" height="52" rx="6" fill="#fff" stroke="#CBD5E1" strokeWidth="1" strokeDasharray="4 3" className="wuv-micro-shimmer" />
          </g>
        </WuvMicroFloatG>

        {/* Divider shimmer */}
        <line x1="280" y1="24" x2="280" y2="276" stroke="#E2E8F0" strokeWidth="1" strokeDasharray="4 4" className="wuv-micro-connector" />

        {/* After — U&V partnership */}
        <rect x="288" y="16" width="256" height="268" rx="12" fill="#F5F3FF" fillOpacity="0.55" stroke="#C4B5FD" strokeWidth="1.2" />
...
function JourneyFeaturedScene({ stageId }: { stageId: WuvPartnerPathStage['id'] }) {
  const scenes: Record<WuvPartnerPathStage['id'], ReactNode> = {
    listen: (
      <>
    ...
    understand: (
      <>
        ...
        <rect x="360" y="40" width="100" height="120" rx="10" fill="#fff" stroke="#7C3AED" strokeWidth="1.2" strokeOpacity="0.3" className="wuv-micro-shimmer" />
        {[0, 1, 2, 3].map((i) => (
          <rect key={i} x={372} y={68 + i * 20} width="76" height="14" rx="7" fill={i === 0 ? '#7C3AED' : '#F1F5F9'} fillOpacity={i === 0 ? 0.2 : 1} stroke="#E2E8F0" strokeWidth="1" />
        ))}
      </>
    ),
    plan: (
          ...
          [0, 1, 2, 3, 4, 5, 6].map((col) => {
            ...
            return (
              <rect
                ...
                style={active ? { animationDelay: `${col * 80}ms` } : undefined}
              />
            );
          }),
        )}
    ...
    build: (
      <>
        ...
        <WuvMicroFloatG delay={200} duration="5.5s">
          <WuvPhone x={380} y={72} />
        </WuvMicroFloatG>
        <rect x="388" y="48" width="72" height="56" rx="8" fill="#fff" stroke="#7C3AED" strokeWidth="1.2" strokeOpacity="0.35" />
        <rect x="396" y="56" width="56" height="8" rx="4" fill="#7C3AED" fillOpacity="0.25" className="wuv-banner-progress" />
      </>
    ),
    launch: (
      <>
        ...
        <WuvMicroFloatG delay={0} duration="4.5s">
          <WuvPhone x={160} y={48} />
        </WuvMicroFloatG>
    ...
    grow: (
      <>
        ...
        <WuvMicroFloatG delay={400} duration="6s">
      ...
      <div className="flex h-full min-h-[180px] flex-col sm:min-h-[200px]">
        ...
        <div className="wuv-journey-banner__track px-3 pb-3 pt-1 sm:px-4 sm:pb-4">
          <div className="relative">
            ...
            <div
              className="wuv-journey-banner__track-fill wuv-micro-track-fill"
      ...
      <svg viewBox="0 0 480 180" className="h-full w-full min-h-[130px]" preserveAspectRatio="xMidYMid slice" aria-hidden>
        {row === 0 && (
          <>
            ...
            {[0, 1, 2].map((i) => (
              <rect key={i} x="40" y={58 + i * 20} width={108} height="14" rx="7" fill={i === 0 ? '#7C3AED' : '#F1F5F9'} fillOpacity={i === 0 ? 0.25 : 1} stroke={i === 0 ? '#7C3AED' : '#E2E8F0'} strokeWidth="1" />
            ))}
...
function IndustryBannerArt({ industry, uid }: { industry: WuvIndustryAnimationId; uid: string }) {
  ...
  const scenes: Record<WuvIndustryAnimationId, ReactNode> = {
    healthcare: (
      <>
...
export function WuvIndustryBanner({ industry }: { industry: WuvIndustryAnimationId }) {
  return (
    <div className="wuv-industry-banner relative h-full min-h-[130px] w-full overflow-hidden sm:min-h-[145px]" role="img" aria-label={`${industry} industry scene`}>
      <IndustryBannerArt industry={industry} uid="ind" />
    </div>
  );
}
